...
router.post('/', auth, async (req, res) => {
  try {
    const { visit_id, contract_id, villa_id, visit_date, visit_type, technician, manager, checklist, notes } = req.body;
    const result = await getDb().query(
      ...
      [visit_id, contract_id, villa_id, visit_date, visit_type, technician, manager, JSON.stringify(checklist), notes]
    );
...
router.put('/:id', auth, async (req, res) => {
  try {
    const { checklist, notes, manager_approved, client_signature, report_url } = req.body;
    await getDb().query(
      'UPDATE service_reports SET checklist=$1,notes=$2,manager_approved=$3,client_signature=$4,report_url=$5 WHERE id=$6',
      [JSON.stringify(checklist), notes, manager_approved, client_signature, report_url, req.params.id]
    );
